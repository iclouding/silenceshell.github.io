...
前面的方案要求先kinit，每个client（机器级别。。。）当前只能有一个用户，其他用户再kinit会顶替掉当前的Principal，不适合多租户和编程场景：
...
基于存储的认证、授权模型基本是可用的了，若辅助以Ranger这样的权限控制，会比较灵活、统一。不过这里还有个问题，Kerberos的用户怎么跟Ranger打通（啊！）。
